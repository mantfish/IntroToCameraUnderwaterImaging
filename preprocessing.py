...
def fix_color(img: np.ndarray, percent: float=0.8):
    """
    Stengthens the color of an image by mapping X percentage highest and lowest color values to either 0 or 255.
    """
    ...
    for channel in cv2.split(img):
        # Create a historgram of the intensity values for each channal in the image
        cumhist = np.cumsum(cv2.calcHist([channel], [0], None, [256], (0, 256)))
        # Define cut off points, i.e 0.8/200 * num of pixels is the cutoff number
        low_cut, high_cut = np.searchsorted(cumhist, cumstops)
        # Create new histogram, the top high_cut number of pixels values are mapped to 255, and vica versa for lowest
        lut = np.concatenate((
            np.zeros(low_cut),
...
def fix_contrast(img: np.ndarray) -> np.ndarray:
    """
    Uses the CLAHE function from openCV to increase the contrast in the image without globally over and underexposling large areas
    """
...
def fix_light(image, limit=3, grid=(7, 7), gray=False):
    """ 
    Uses the CLAHE function from openCV to increase the contrast of only the luminosity channel of the image
    """
